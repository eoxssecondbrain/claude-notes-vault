...
1. On the FIRST turn of a new conversation only: silently choose a short, stable `thread_name` (e.g. `sabre-alloys-payment-delay`) describing the conversation's topic. **This value is bound to the CONVERSATION, not to the current subject matter of any individual message.** Once chosen, it never changes for the rest of this conversation — not for gradual topic drift, and not for a hard, unrelated jump to a completely different subject (e.g. the conversation started about incentive requests and the user suddenly asks "who is Sabre Alloys" — that is still the SAME conversation, so it is still the SAME `thread_name`). Do not re-evaluate whether the name still "fits" the latest message; it was never meant to describe every message, only to identify which file this conversation's transcript lives in. A changed `thread_name` creates a second file and silently orphans the first — that is a bug every time it happens, with no exception for how unrelated the new message seems.
2. Compose your actual response to the user as normal.
3. **Before ending your turn — not after, not "when convenient," not "if it seems important" — call `save_chat_transcript(thread_name, new_messages=<the user's latest prompt and your response>)`.** The server will append this to the existing file. Do NOT send the full transcript, only the new messages. This is turn N+1's action even if turn N already saved — every turn, without exception, gets its own call.
...
**Why this needs to be this explicit:** two documented, observed failure modes.

First: a skill correctly saving on the FIRST turn of a conversation (because that's when the skill gets read and its instructions are freshest), then never saving again for the rest of that same conversation — as if the rule were a one-time setup step rather than a standing, per-turn obligation. If you notice yourself treating "I already saved once in this conversation" as satisfying this rule, that reasoning is wrong — re-read step 3 above. Every turn is independent.

Second, and separately: a conversation saving reliably turn after turn while the topic stays roughly related, then going completely silent the moment the user asks something with NO topical connection to what came before (e.g. a conversation about incentive requests, then a sudden "who is Sabre Alloys"). This happens because the model treats the hard subject-matter jump as a cue to reconsider `thread_name` — "does this still describe what's being discussed?" — and that reconsideration itself breaks the chain, whether it results in silently skipping the save, or picking a new name and starting an orphaned second file that also stops getting updated. `thread_name` is a conversation identifier, not a topic label — an unrelated question mid-conversation is not a signal to touch it. Keep saving to the exact same file no matter how unrelated the latest message is to the first one.

**This appends to the same file each time** — one file per conversation, not one file per message. See the tool doc below for exactly how the filename is derived.
...
- **`save_chat_transcript(thread_name, new_messages)`** — saves/updates a conversation in `raw/claude-chat-queries/`, commits + pushes to this vault's own `data` branch. **Automatic per the rule above — never ask first, never skip.**
  ...
  - `<created-date>` is fixed at the thread's first save and does not change on later appends, even if a save happens on a different calendar day than the first one.
  - Every call for the same `thread_name` (from the same user) appends to that one file — it does not create `-2`, `-3`, etc. Changing `thread_name` mid-conversation is what would incorrectly create a second file; keep it stable.
  - Always pass **only the new messages**, not the full transcript — the tool appends this to the file's content.
